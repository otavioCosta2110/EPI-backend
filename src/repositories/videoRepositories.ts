...
export default class VideoRepository {
  ...
  createVideo = async (video: VideoModel): Promise<VideoModel> => {
    ...
    try {
      ...
      const videoResult = await client.query(
        'INSERT INTO videos (id, title, url, description, user_id, rating, timesRated, ratingtotal) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id',
        [
          video.id,
          video.title,
          video.url,
  ...
  deleteVideo = async (videoID: string): Promise<boolean> => {
    ...
    try {
      ...
      const result = await client.query(
        'UPDATE videos SET is_deleted = TRUE WHERE id = $1 AND is_deleted = FALSE',
        [videoID]
      );
      await client.query('COMMIT');
  ...
  rateVideo = async (userID: string, videoID: string, newRating: number) => {
    try {
      ...
      const resultAverage = await pool.query(
        'UPDATE videos SET rating = $1 WHERE id = $2',
        [average, videoID]
      );
      const resultTimesRated = await pool.query(
        'UPDATE videos SET timesrated = timesrated + 1 WHERE id = $1',
        [videoID]
      );
      const resultRatingTotal = await pool.query(
        'UPDATE videos SET ratingtotal = ratingtotal + $1 WHERE id = $2',
        [newRating, videoID]
      );
      const resultRatingUserVideos = await pool.query(
        'UPDATE user_videos SET rating = $1 WHERE user_id = $2 AND video_id = $3',
        [newRating, userID, videoID]
      );
    } catch (err) {
      throw err;
    }
  ...
  isVideoRatedByUser = async (
    userID: string,
    videoID: string
  ): Promise<boolean> => {
    const result = await pool.query(
      'SELECT * FROM user_videos WHERE user_id = $1 AND video_id = $2 AND rating IS NOT NULL',
      [userID, videoID]
    );
    console.log(result);
    if (result.rows.length === 0) {
